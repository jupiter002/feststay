...
def getdistance(fesname):
    idx = []
    x_1, y_1, name, si, place = getfesdot(fesname)
...
smonth = 1 if month=='1월' else \
        ...
        8 if month=='8월' else \
        9 if month=='9월' else \
        10 if month=='10월' else \
        11 if month=='11월' else 12
...
if select2 == '모텔':
    ...
    df2 = pd.read_csv('./data/recom_rest/모텔_list_추천여부최종최종.csv')
...
# 펜션을 선택할시
    ...
    df2 = pd.read_csv('./data/recom_rest/펜션_list_추천여부최종최종.csv')
...
try:
    ...
    with left_column:
        ...
        # Folium 지도 생성
        m = folium.Map(location=[lat, lon], zoom_start=9.5)
        ...
        for _, row in df.iterrows():
            ...
            # 숙소 마커 추가
            folium.Marker([motel_lat, motel_lon], tooltip=motel_name, icon=folium.Icon(color='blue')).add_to(marker_cluster)
    ...
    with right_column:
        ...
        sorted_df = merged_df.sort_values(by=['추천여부_최종', opt], ascending=[False, False])
